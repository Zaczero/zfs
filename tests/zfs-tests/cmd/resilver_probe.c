...
main(int argc, char **argv)
{
	...
	VERIFY(strcmp(mode, "reopen") == 0 || strcmp(mode, "clear") == 0 ||
	    strcmp(mode, "probe") == 0);
	...
	VERIFY(!scn->scn_dtl_valid);
	/* Both callbacks must count; a lost increment would leave only one. */
	VERIFY3U(scn->scn_phys.scn_errors, ==, 2);
	VERIFY(dsl_scan_resilver_scheduled(dp));

	/*
	 * Traversal is still suspended. The availability/configuration sync
	 * must persist these errors without waiting for scan progress.
	 */
	txg_wait_synced(dp, 0);
