...
for command in online scrub errorscrub reopen clear; do
	...
	if [[ "$defer" == enabled ]]; then
		...
		# Without deferral, persist the coverage failure or restart with
		# a minimum that includes the returning leaf's excluded range.
		log_must eval "zdb -dddd '$TESTPOOL1' 1 >'$workdir/scan'"
		# shellcheck disable=SC2016 # awk field references
		log_must awk -v max="$dtl_max" '$1 == "scan" {
		    found = ($3 == 2 && $4 == 1 && ($16 > 0 || $6 < max))
		} END { exit !found }' "$workdir/scan"
	fi
	log_must zinject -c all
	log_must set_tunable32 SCAN_SUSPEND_PROGRESS 0
