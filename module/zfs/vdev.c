	...
	if (vd->vdev_ops->vdev_op_leaf) {
		...
		if (rebuild_done &&
			...
			check_excise = (scn != NULL &&
			    ...
			    (scn->scn_dtl_valid || zfs_scan_ignore_errors));
		}

		if (scrub_txg && check_excise &&
		    vdev_dtl_should_excise(vd, rebuild_done)) {
			/*
			 * Healing accounted for missing data up to scrub_txg.
			 * Excise that region and fold in exact failed repairs.
			 *
...
/*
 * Determine if resilver is needed and return any requested TXG bounds.
 */
boolean_t
	...
	if (type == ZIO_TYPE_WRITE && txg != 0 &&
		...
		if (vd->vdev_ops->vdev_op_leaf) {
			...
			if (flags & ZIO_FLAG_SCAN_THREAD) {
				...
				dsl_scan_count_error(
				    spa->spa_dsl_pool->dp_scan, B_TRUE);
